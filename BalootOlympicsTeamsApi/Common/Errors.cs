...
public abstract class ResultError(string msg, ErrorType errorCode, int statusCode) : Error(msg)
{
    ...
    public virtual IResult ToIResult(string traceId) =>
       Results.Json(new ErrorResponse(ErrorCode, Message, traceId), statusCode: StatusCode);
...
public class InvalidBodyInputError(Dictionary<string, List<string>> validationErrors, string msg = "Invalid Body Input") : ResultError(msg, ErrorType.InvalidBodyInput, StatusCodes.Status400BadRequest)
{
    ...
    public override IActionResult ToIActionResult(string traceId)
    {
        return new JsonResult(new ValidationErrorResponse(ErrorCode, Message, ValidationErrors)) { StatusCode = StatusCode };
    }
    public override IResult ToIResult(string traceId)
    {
        return Results.Json(new ValidationErrorResponse(ErrorCode, Message, ValidationErrors), statusCode: StatusCode);
    }
}
    ...
    StatusCodes.Status409Conflict)
{
    ...
    public override IResult ToIResult(string traceId)
    {
        return Results.Json(new ValidationErrorResponse(ErrorCode, Message, new Dictionary<string, List<string>>() {
            {PropertyName, [Message]}
        }), statusCode: StatusCode);
    }
}
